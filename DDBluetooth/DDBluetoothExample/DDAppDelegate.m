...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    dispatch_queue_t centralQueue = dispatch_queue_create("com.DDBluetooth.corebluetooth", DISPATCH_QUEUE_SERIAL);
    [[DDCentralManager sharedManager] initialize:centralQueue options:options didInitialize:^(CBCentralManagerState state) {
        switch (state) {
            case CBCentralManagerStatePoweredOn:
                break;
            default:
                NSLog(@"Bluetooth is currently not available on this device");
                break;
        }
    }];
